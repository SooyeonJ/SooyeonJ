# 👩‍💻 SOOYEON JEONG

🎓 M.Eng Hanyang University in Computer Science, Sep 2025 – now 🎓 B.S. in Computer Software Engineering  

💼 IT System Operator specialized in **Healthcare Data** · **DevOps** · **CI/CD** · **Database Operation**

---

## 💼 Career

| Company | Duration |
|---------|----------|
| Korea Health Information Service (KHIS) | 2023.08 ~ now |
| Yonsei University Health System | 2022.08 ~ 2023.07 |
| UBcare | 2021.06 ~ 2022.07 |
| SEESAWTALK | 2019.04 ~ 2019.12 |

---
## 🚀 Featured Projects & Products

#### 🏥 [Healthcare MyData System](https://www.myhealthway.go.kr) — Korea Health Information Service
- 국가 건강정보 고속도로 인프라 운영 · [보도](http://www.bosa.co.kr/news/articleView.html?idxno=2228925)
- 상급종합병원 47개소·종합병원 전체 [FHIR](https://www.hl7.org/fhir/) 연계 완료
  - 권역별 클라우드(NIA·네이버·카카오·더존) 관리
- 나만의닥터·카카오헬스케어·마이웰플러스 등 활용기관 SDK 배포 및 연계 관리
- Jenkins·GitLab 기반 CI/CD 파이프라인 운영

#### 💊 의사랑 EMR 운영 및 기능 개발 — UBcare
- 국내 1위 EMR 의사랑 시스템 운영 및 유지보수 · 기능 개선
- eGFR 자동계산 기능 개발 — 혈청크레아티닌 입력 시 나이·성별 연동 자동 산출
  - 한국아스트라제네카 후원 · 2021.11 출시 · [의학신문](https://www.bosa.co.kr/news/articleView.html?idxno=2161395) · [라포르시안](https://www.rapportian.com/news/articleView.html?idxno=140213)

#### 📈 GAPS ETF Portfolio Competition — [DB Securities](https://dbgaps.dbsec.co.kr)
- DB증권 ETF 포트폴리오 운용 대회
- 퀀트 담당 · 매크로-모멘텀-퀀트 전략 수치화 및 백테스팅
- pykrx 기반 ETF 188개 데이터 수집
---

## 📄 Research

### [Identifying Contact Time Required for Secondary Transmission of *Clostridioides difficile* Infections by Using Real-Time Locating System](https://wwwnc.cdc.gov/eid/article/30/5/23-1588_article)
*Emerging Infectious Diseases, CDC — Vol.30, No.5, May 2024*
🏅 **Published in SCI-level International Journal (CDC, IF 7.2)**

- 연세의료원 RTLS 태그 데이터 기반 index patient 26명 · 접촉 사례 4,196건 분석
- 결측값·이상치 처리 및 추적 기간 필터링 → 최종 3,620건 분석 데이터셋 구축
- 접촉 시간과 2차 감염 발생 간 상관관계 도출 위한 변수 정의 및 피처 엔지니어링

---

## 🎯 Interests

`Data Engineering` `DevOps` `System Reliability` `Backend Operation` `Financial IT`

---

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=SooyeonJ&show_icons=true&theme=default&hide_border=true" />
</p>
